...
export class ChainUserFetcher extends ChainFetcher implements UserFetcher {
  ...
  async fetchUserMarketData(
    ...
    const [
      ...
      erc20.allowance(userAddress, CONTRACT_ADDRESSES.bulker, overrides),
  ...
  async fetchStethData(userAddress: Address, blockTag: BlockTag = "latest") {
    ...
    const [
      ...
      stEth.getPooledEthByShares(WadRayMath.WAD, overrides),
      stEth.allowance(userAddress, CONTRACT_ADDRESSES.permit2, overrides),
      stEth.allowance(userAddress, CONTRACT_ADDRESSES.bulker, overrides),
      this._permit2!.allowance(
        userAddress,
        addresses.steth,
        CONTRACT_ADDRESSES.bulker,
